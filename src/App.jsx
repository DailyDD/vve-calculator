...
function exportPDF(r) {
  ...
  const html = '<!DOCTYPE html><html lang="nl"><head><meta charset="UTF-8"><title>VvE Bijdrage – ' + r.complexNaam + '</title>'
    ...
    + '.sec{font-size:8pt;font-weight:700;text-transform:uppercase;letter-spacing:.08em;color:#8A7E7B;margin:18px 0 8px;padding-bottom:4px}'
    ...
    + 'tfoot td{padding:7px 10px;font-weight:600;color:#1A1614;border-top:2px solid #1A1614;background:#fff}'
    ...
    + '.page-logo{position:fixed;top:16px;right:20px;height:44px;opacity:.9}'
    + '@media print{body{padding:16px 20px}.print-btn{display:none}.page-logo{display:block}body{-webkit-print-color-adjust:exact;print-color-adjust:exact}}'
    + '</style></head><body>'
    + '<img src="data:image/png;base64,' + LOGO + '" class="page-logo" alt="Totaal VvE Beheer">'
    + '<button class="print-btn" onclick="window.print()">🖨 Afdrukken / PDF</button>'
    + '<div class="hdr"><div><h1>' + (r.alleenEenmalig ? 'Eenmalige Bijdrage Rapport' : 'Reservefonds Bijdrage Rapport') + '</h1><div class="meta">' + r.complexNaam + ' · Opgesteld op ' + today() + '</div></div><img src="data:image/png;base64,' + LOGO + '" alt="Totaal VvE Beheer" style="height:52px"></div>'
    + '<div class="intro"><strong>' + r.complexNaam + '</strong><br>' + (r.alleenEenmalig ? 'Berekening eenmalige bijdragen per eigenaar — opgesteld ' + today() + '.' : 'Berekening minimale maandelijkse bijdrage reservefonds conform art. 5:126 BW — opgesteld ' + today() + '.') + '</div>'
    ...
    + (r.alleenEenmalig ? '' : '<div class="note"><strong>Toelichting:</strong> Methode 1 (MJOP) verdient de voorkeur bij een actueel MJOP (&lt;5 jaar oud). Methode 2 (0,5%) is het wettelijk minimum conform art. 5:126 lid 3 BW (v.a. 1 jan 2021). Genoemde bedragen zijn minimale bijdragen — een aanvullende buffer is aan te raden.</div>')
      ...
      <div style={{ maxWidth: 960, margin: '0 auto', padding: '28px 20px 80px' }}>
        ...
        {result && (
          <div id="res-anker">
            ...
            {/* EENMALIGE BIJDRAGEN RESULTAAT */}
            {result.eenmaligAan && result.eenmaligBerekend && result.eenmaligBerekend.length > 0 && (
              <div style={{ marginTop: 16 }}>
                <SecTitle>Eenmalige bijdragen per eigenaar</SecTitle>
                      ...
                      <div style={{ padding: '12px 20px', background: S.cream, display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                        ...
                        <div style={{ textAlign: 'right' }}>
                          <span style={{ fontFamily: 'Georgia,serif', fontSize: 18, color: item.tekort > 0 ? S.bordeaux : S.green }}>
                            {item.tekort > 0 ? 'Tekort: ' + new Intl.NumberFormat('nl-NL',{style:'currency',currency:'EUR'}).format(item.tekort) : '✓ Volledig gedekt door reserve'}
                          </span>
                        </div>
                      </div>
