...
function exportPDF(r) {
  ...
  const html = '<!DOCTYPE html><html lang="nl"><head><meta charset="UTF-8"><title>VvE Bijdrage \u2013 ' + r.complexNaam + '</title>'
    ...
    + '</div>'
    + '<div class="block"><div class="bh"><div class="tag">Totale jaarlasten VvE</div><div class="name">Incl. exploitatiekosten</div></div>'
    + rr('0,5% reservering (jaarlijks)', fmt(r.jaar05)) + rr('Exploitatiekosten', fmt(r.exploitatie)) + rr('Totale jaarlasten VvE', fmt(r.jaar05Totaal)) + rrB('Maandlasten VvE totaal', r.has05 ? fmt(r.mnd05) : '\u2014')
    + '</div></div>'
  ...
  return (
    <div>
      ...
      <div style={{ maxWidth: 960, margin: '0 auto', padding: '28px 20px 80px' }}>
        ...
        <Card header={<CardHdr icon="💼" bg={S.blueBg} title="Exploitatiekosten" sub="Buiten het MJOP — optioneel maar van invloed op totale bijdrage" />}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 14, padding: '18px 20px' }}>
            ...
            <Field label="Overig (€/jaar)"><Inp type="number" placeholder="bijv. 1800" value={overig} onChange={e => setOverig(e.target.value)} /></Field>
          </div>
        </Card>
